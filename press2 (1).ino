// press2.ino
// Weekopdracht 2 ESP:Sayid Abd-Elaziz & Davy van Weijen
//
#include <WiFi.h>
#include <esp_sntp.h>
#include "secret.h"
...
void setup()
{
 ...
 tzset();
  ...
  // create audit queue
  auditQueue = xQueueCreate(200, sizeof(char *));
  assert(auditQueue);
}
...
void addAuditQueue(){
    ...
    if(uxQueueSpacesAvailable(auditQueue) == 0){
        // remove oldest timestamp
        char *oldest;
        xQueueReceive(auditQueue, &oldest, 0);
        free(oldest);
    }

    // add new timestamp
    char *timestamp = printLocalTime();
    xQueueSendToBack(auditQueue, &timestamp, 0);
}
...
void printAuditQueue(){
    // print queue
    char *timestamp;
    while(xQueueReceive(auditQueue, &timestamp, 0) == pdPASS){
